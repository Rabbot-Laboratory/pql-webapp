...
(追記 2026-08-30: スライダーでtargetを上げたときの実機の曲がりが3Dプレビューと全8関節で逆だと判明。`joint_preview` の符号(股−1/膝+1 → 股+1/膝−1)と `ACTUATOR_HEIGHT_EFFECTS` の全符号を反転。本ドキュメントの符号議論は「target増=伸長」の前提は維持されるが、幾何符号は反転後の値が正。ミキシング行列の実機方向確認(Phase 9)は未実施のままなので、次回セッションで手動傾斜テストを行うこと。)
