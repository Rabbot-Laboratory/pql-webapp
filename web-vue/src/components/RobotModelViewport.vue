...
camera.up.set(0, 0, 1);
camera.position.set(-0.95, 0.7, 0.55);
...
function updateCylinderBindings(): void {
  ...
  for (const binding of cylinderBindings) {
    ...
    const currentLength = cylinderDirection.length();
...
function classifyCadLeg(root: THREE.Object3D): LegId {
  ...
  // In the colored STEP assembly the physical nose is on +Y. The legacy URDF
  // joint names use the opposite longitudinal sign, so classifying the CAD
  // roots with the URDF sign makes front actuator data animate the rear
  // geometry (and vice versa). Keep actuator/joint IDs unchanged and correct
  // only the CAD-to-leg visual assignment here.
  const longitudinal = center.y > 0 ? 'front' : 'rear';
  const lateral = center.x > 0 ? 'right' : 'left';
  return `${longitudinal}_${lateral}` as LegId;
}

function replaceCadFootPart(
  targetLowerCad: THREE.Object3D,
  donorLowerCad: THREE.Object3D,
  namePrefix: string,
  donorToTarget: THREE.Matrix4,
): THREE.Object3D {
  const targetPart = findNamedDescendant(targetLowerCad, namePrefix);
  const donorPart = findNamedDescendant(donorLowerCad, namePrefix);
  const targetParent = targetPart?.parent;
  if (!targetPart || !donorPart || !targetParent) {
    throw new Error(`CAD foot transplant part is missing: ${namePrefix}`);
  }

  donorPart.updateWorldMatrix(true, true);
  targetParent.updateWorldMatrix(true, false);

  const desiredWorld = new THREE.Matrix4().multiplyMatrices(
    donorToTarget,
    donorPart.matrixWorld,
  );
  const desiredLocal = new THREE.Matrix4()
    .copy(targetParent.matrixWorld)
    .invert()
    .multiply(desiredWorld);

  const replacement = donorPart.clone(true);
  replacement.name = targetPart.name;
  desiredLocal.decompose(
    replacement.position,
    replacement.quaternion,
    replacement.scale,
  );
  targetParent.add(replacement);
  replacement.updateMatrixWorld(true);

  targetParent.remove(targetPart);
...
function transplantFrontRightFootTip(
  byLeg: Map<LegId, THREE.Object3D>,
  cadScene: THREE.Group,
): void {
  const targetLeg = byLeg.get('front_right');
  // Front-right and rear-left use the same non-mirrored lower-leg part family
  // in the STEP assembly. The other two legs use the mirrored family.
  const donorLeg = byLeg.get('rear_left');
  const targetLowerCad = targetLeg
    ? findNamedDescendant(targetLeg, 'PQL01-LD00-A1')
    : null;
  const donorLowerCad = donorLeg
    ? findNamedDescendant(donorLeg, 'PQL01-LD00-A1')
    : null;
  if (!targetLowerCad || !donorLowerCad) {
    throw new Error('CAD front-right foot transplant lower-leg source is missing');
  }

  // Rigid transform from the rear-left donor into the front-right placement.
  // It was solved from the two foot meshes' vertex clouds in the source GLB.
  // Reverse nearest-neighbour error is below 0.00001 mm, unlike the earlier
  // three-anchor bounding-box fit which left a visible ~2.7 mm rotation error.
  const donorToTargetCad = new THREE.Matrix4().set(
    -0.999939484776, 0.011001137158, 0.000042028050, -0.001418492178,
    0.011001090710, 0.999939006114, -0.000979824054, 0.001697014836,
    -0.000052804666, -0.000979302405, -0.999999519089, -0.013128772852,
    0, 0, 0, 1,
  );
  cadScene.updateWorldMatrix(true, false);
  const cadWorld = cadScene.matrixWorld.clone();
  const donorToTarget = cadWorld
    .clone()
    .multiply(donorToTargetCad)
    .multiply(cadWorld.invert());
...
function attachColoredCadModel(nextRobot: URDFRobot, cadScene: THREE.Group): void {
  ...
  transplantFrontRightFootTip(byLeg, cadScene);
    ...
    for (const cylinderRoot of cylinderRoots) {
      ...
      try {
        cylinderBindings.push(
          createCylinderBinding(cylinderRoot, proximalLink, distalLink),
        );
      } catch (cylinderError) {
...
async function loadRobotModel(): Promise<void> {
  ...
  try {
    ...
    console.warn('Colored CAD model failed; falling back to legacy STL visuals.', cadError);
    cylinderBindings.length = 0;
    contactFootObjects.clear();
    disposeRobot(coloredRobot);
    cadScene?.parent?.removeFromParent();
    nextRobot = await parseLegacyRobot(xml);
    robotRoot.add(nextRobot);
    configureRobotMaterials(nextRobot);
  }
